...
class OfflinePitchAnalyzer:
    ...
    def analyze_file(
        self, audio_file: pathlib.Path
    ) -> tuple[torch.Tensor, torch.Tensor]:
        stems = self._track_separator.separate_file(audio_file)
        return self._detect(stems)

    def _detect(
        self, stems: dict[str, torch.Tensor]
    ) -> tuple[torch.Tensor, torch.Tensor]:
        vocals = stems["vocals"]
        return self._pitch_detector.detect_pitch(
            vocals, self._track_separator.get_sample_rate()
        )


class StreamingPitchAnalyzer:
    def __init__(
        self, pitch_detector: PitchDetector, window_sec: float = 0.20, sr: int = 44100
    ):
        self._pitch_detector = pitch_detector
